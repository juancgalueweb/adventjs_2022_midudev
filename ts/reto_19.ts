...
function sortToys(toys: string[], positions: number[]): string[] {
  type GenObject = { [key: string]: number };
  const garage: GenObject = {};
  toys.forEach((_, index) => (garage[toys[index]] = positions[index]));
  return Object.keys(garage).sort(function (a, b) {
    return garage[a] - garage[b];
  });
}

console.log(sortToys(['ball', 'doll', 'car', 'puzzle'], [2, 3, 1, 0])); //['puzzle', 'car', 'ball', 'doll']
console.log(
