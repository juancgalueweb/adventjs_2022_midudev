...
El archivo con id 1 ha sido modificado dos veces después del último backup.
...
Tenemos que hacer una copia de seguridad de los archivos 1 y 3.
...
function getFilesToBackup(lastBackup: number, changes: number[][]): number[] {
  const result = changes
    .filter((change) => change[1] - lastBackup > 0)
    .map((ele) => ele[0]);
